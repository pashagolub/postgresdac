...
{
	char buf[1024]; //mi buffer for string formatting
	...
	if (LogCallBackProc) //mi
	{
		vsprintf(buf,  _(fmt), ap);
		LogCallBackProc(buf);
	}	
}
...
exit_nicely(int code)
{
	...
	if (code > 0) mi_die_not_so_horribly(code); //mi, next line should never happen
	//exit(code);
}
