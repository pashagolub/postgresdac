...
type
  ...
  TPSQLUpdateSQLEditForm = class(TForm)
    OkButton: TButton;
  ...
  private
    StmtIndex: Integer;
    DataSet: TPSQLDataSet;
    Database: TPSQLDatabase;
    FTempTable : TPSQLTable;
    DatabaseOpened: Boolean;
    UpdateSQL: TPSQLUpdateSQL;
...
function EditPSQLUpdateSQL(AUpdateSQL: TPSQLUpdateSQL): Boolean;

implementation

{$R *.DFM}

uses Dialogs, BdeConst, LibHelp, TypInfo, BDE, PSQLTypes;

{ Global Interface functions }

function EditPSQLUpdateSQL(AUpdateSQL: TPSQLUpdateSQL): Boolean;
begin
  with TPSQLUpdateSQLEditForm.Create(Application) do
  try
  ...
  case P^ of
    ...
    '''', '"':
      begin
        ...
        TokenStart := P;
        while not CharInSet(P^, [QuoteChar, #0]) do Inc(P);
        SetString(FTokenString, TokenStart, P - TokenStart);
        Inc(P);
        Trim(FTokenString);
        FToken := stSymbol;
...
function TPSQLUpdateSQLEditForm.DatabaseOpen: Boolean;
begin
  ...
  begin
    ...
    //New
    if not Assigned(DataSet.Database) then Exit;
...
function TPSQLUpdateSQLEditForm.Edit: Boolean;
var
  Index: TUpdateKind;
  DataSetName: string;
begin
  Result := False;
  if Assigned(UpdateSQL.DataSet) and (UpdateSQL.DataSet is TPSQLDataSet) then
  begin
    DataSet := TPSQLDataSet(UpdateSQL.DataSet);
...
procedure TPSQLUpdateSQLEditForm.GenWhereClause(const TabAlias, QuoteChar: string;
  KeyFields, SQL: TStrings);
var
  I: Integer;
  BindText: string;
...
procedure TPSQLUpdateSQLEditForm.GenDeleteSQL(const TableName, QuoteChar: string;
  KeyFields, SQL: TStrings);
begin
  SQL.Clear;
...
procedure TPSQLUpdateSQLEditForm.GenInsertSQL(const TableName, QuoteChar: string;
  UpdateFields, SQL: TStrings);
...
procedure TPSQLUpdateSQLEditForm.GenModifySQL(const TableName, QuoteChar: string;
  KeyFields, UpdateFields, SQL: TStrings);
var
  I: integer;
  Comma: string;
...
procedure TPSQLUpdateSQLEditForm.GenerateSQL;
  ...
  begin
    with UpdateTableName do
      if ((ItemIndex <> -1) and (Items.Objects[ItemIndex] <> nil)) or
         (DatabaseOpen and not True and (Pos('.', BaseName) > 0)) then
...
procedure TPSQLUpdateSQLEditForm.GetDataSetFieldNames;
begin
...
procedure TPSQLUpdateSQLEditForm.GetTableFieldNames;
begin
...
function TPSQLUpdateSQLEditForm.GetTableRef(const TabName, QuoteChar: string): string;
begin
//  if QuoteChar <> '' then
//    Result :=  TabName + '.' else
  REsult := '';
...
procedure TPSQLUpdateSQLEditForm.InitGenerateOptions;
var
  UpdTabName: string;
...
procedure TPSQLUpdateSQLEditForm.InitUpdateTableNames;
begin
  UpdateTableName.Items.Clear;
  if Assigned(DataSet) then
  begin
    if DataSet is TPSQLQuery then
      GetSQLTableNames(TPSQLQuery(DataSet).SQL.Text, UpdateTableName.Items)
    else if (DataSet is TPSQLTable) and (TPSQLTable(DataSet).TableName <> '') then
      UpdateTableName.Items.Add(TPSQLTable(DataSet).TableName);
  end;
  if UpdateTableName.Items.Count > 0 then
     UpdateTableName.ItemIndex := 0;
end;

procedure TPSQLUpdateSQLEditForm.SetButtonStates;
begin
...
procedure TPSQLUpdateSQLEditForm.SelectPrimaryKeyFields;
var
...
procedure TPSQLUpdateSQLEditForm.SetDefaultSelections;
var
...
procedure TPSQLUpdateSQLEditForm.ShowWait(WaitMethod: TWaitMethod);
begin
...
function TPSQLUpdateSQLEditForm.TempTable: TPSQLTable;
begin
...
procedure TPSQLUpdateSQLEditForm.HelpButtonClick(Sender: TObject);
begin
  Application.HelpContext(HelpContext);
end;

procedure TPSQLUpdateSQLEditForm.StatementTypeClick(Sender: TObject);
begin
...
procedure TPSQLUpdateSQLEditForm.OkButtonClick(Sender: TObject);
begin
  if SQLMemo.Modified then
    SQLText[TUpdateKind(StmtIndex)].Assign(SQLMemo.Lines);
end;

procedure TPSQLUpdateSQLEditForm.DefaultButtonClick(Sender: TObject);
begin
  with UpdateTableName do
...
procedure TPSQLUpdateSQLEditForm.GenerateButtonClick(Sender: TObject);
begin
  GenerateSQL;
  FSettingsChanged := False;
end;

procedure TPSQLUpdateSQLEditForm.PrimaryKeyButtonClick(Sender: TObject);
begin
  ShowWait(SelectPrimaryKeyFields);
  SettingsChanged(Sender);
end;

procedure TPSQLUpdateSQLEditForm.PageControlChanging(Sender: TObject;
  var AllowChange: Boolean);
begin
...
procedure TPSQLUpdateSQLEditForm.FormDestroy(Sender: TObject);
begin
  if DatabaseOpened then
    Database.Close;
end;

procedure TPSQLUpdateSQLEditForm.GetTableFieldsButtonClick(Sender: TObject);
begin
...
procedure TPSQLUpdateSQLEditForm.SettingsChanged(Sender: TObject);
begin
...
procedure TPSQLUpdateSQLEditForm.FormCloseQuery(Sender: TObject;
...
procedure TPSQLUpdateSQLEditForm.UpdateTableNameChange(Sender: TObject);
begin
  SettingsChanged(Sender);
end;

procedure TPSQLUpdateSQLEditForm.UpdateTableNameClick(Sender: TObject);
begin
  if not Visible then Exit;
  GetTableFieldsButtonClick(Sender);
end;

procedure TPSQLUpdateSQLEditForm.SelectAllClick(Sender: TObject);
begin
  SelectAll(FieldListPopup.PopupComponent as TListBox);
end;

procedure TPSQLUpdateSQLEditForm.ClearAllClick(Sender: TObject);
var
  I: Integer;
begin
...
procedure TPSQLUpdateSQLEditForm.SQLMemoKeyPress(Sender: TObject;
  var Key: Char);
begin
