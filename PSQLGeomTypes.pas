...
begin
  ...
  begin
    Result.LowerBound.State := TPSQLRangeBoundState(ifthen(Value[START_STR_INDEX] = '[', ord(rbsInclusive), ord(rbsExclusive)));
    Result.UpperBound.State := TPSQLRangeBoundState(ifthen(Value[Length(Value){$IFDEF ZEROBASEDSTRINGS}-1{$ENDIF}] = ']', ord(rbsInclusive), ord(rbsExclusive)));
